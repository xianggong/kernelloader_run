...
run:
	test ! -f sim.rpt && find -name "*.ini" -exec bash -c "$(M2SROOT)/bin/m2s --si-sim detailed --si-max-cycles $(MAX_CYCLES) --mem-report mem.rpt --si-debug-scheduler sch.rpt --opencl-debug stdout --si-report sim.rpt --si-config $(M2SROOT)/samples/southern-islands/7970/si-config --mem-config $(M2SROOT)/samples/southern-islands/7970/mem-config ../../../../../build/loader/src/loader_m2s --logtostderr=1 --config={} > log 2>&1 " \;
